...
class Tracker:
    def __init__(
            ...
            smooth_feature_gamma: float = 0.9,box_cur=[],trace=[],label="",score=0,id=0):
        self.id = str(id)
    ...
    def update(self, detection: Detection):
        self.steps_positive += 1
        self.box_cur=detection.box
        #self.trace.append(((self.box_cur[0]+self.box_cur[2])//2,(self.box_cur[1]+self.box_cur[3])//2))
        # if(len(self.trace)>5):
        #     del self.trace[0]
        # KF tracker update for position and size
        z = self.model.box_to_z(detection.box)
        self._tracker.update(z)
        if(self.score<detection.score):
...
class Motpy:
    ...
    def update(self, box_detects, scores,classes) -> Sequence[Track]:
        ...
        for det_idx in set(range(len(detections))).difference(assigned_det_idxs):
            self.id+=1
            tracker = Tracker(box0=detections[det_idx].box,box_cur=detections[det_idx].box,label=detections[det_idx].label,
